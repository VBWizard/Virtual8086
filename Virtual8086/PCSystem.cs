...
namespace VirtualProcessor
{
    ...
    public class cBreakpoint
    {
        ...
        public bool DebugToScreen { get; set; }
        public String TaskName { get; set; }
        public cBreakpoint(UInt32 pCS, UInt32 pEIP, bool pDisableOnHit, bool pRemoveOnHit, bool ToScreen, bool ToFile)
        {
            CS = pCS;
            EIP = pEIP;
            ...
            Enabled = true;
            DebugToFile = ToFile;
            DebugToScreen = ToScreen;
        }
        public cBreakpoint(byte SoftIntNum, byte Function, bool DOSFunction, bool ToScreen, bool ToFile)
        {
        ...
        public cBreakpoint(UInt32 Task, bool pDisableOnHit, bool ToScreen, bool ToFile)
        {
            TaskNum = Task;
            DisableOnHit = pDisableOnHit;
            DebugToFile = ToFile;
        ...
        public cBreakpoint(String pTaskName, bool pDisableOnHit, bool ToScreen, bool ToFile)
        {
            TaskName = pTaskName;
            DisableOnHit = pDisableOnHit;
            DebugToFile = ToFile;
    ...
    public class PCSystem
    {
        ...
        internal bool bFPUEnabled = false;
        public void FPUEnabled(bool Enabled)
        {
            bFPUEnabled = Enabled;
        }
        ...
        public eOpSysType OSType = eOpSysType.Other;
            ...
            if (!PoweredUp)
                switch (RomType)
                {
                    case eRomTypes.BIOS:
                        ...
                        {
                            BiosFile.Read(PhysicalMem.mMemBytes, 0xf0000, (int)BiosFile.Length);
                            bigBIOS = false;
                        }
                        else
                        {
                            BiosFile.Read(PhysicalMem.mMemBytes, 0xE0000, (int)BiosFile.Length);
                            bigBIOS = true;
                        }
                        BiosFile.Close();
                        break;
                    case eRomTypes.Video:
                        ...
                        VideoFile.Read(PhysicalMem.mMemBytes, 0xC0000, (int)VideoFile.Length);
                        VideoFile.Close();
                        break;
                    case eRomTypes.Misc:
                        mMiscRomPathFile = Path;
                        FileStream lTempFile = new FileStream(Path, FileMode.Open, FileAccess.Read);
                        lTempFile.Read(PhysicalMem.mMemBytes, MiscRomLoadAddr, (int)lTempFile.Length);
                        lTempFile.Close();
                        break;
                }
            else
        ...
        public void PrintDebugMsg(eDebuggieNames Source, String Message)
        {
            ...
            if (Processor_80x86.mCurrInstructOpMode == ProcessorMode.Protected)
                lMessage2 = "\t\t\t\t CPU currently at: " + mProc.regs.CS.DescriptorNum.ToString("X4") + ":" + mProc.regs.EIP.ToString("X8") + "(" + mProc.sCurrentDecode.InstructionAddress.ToString("X8") + ")";
            else
