...
//            SetFlagsForAddition(mProc, lPreVal1, Op2Value, lOp1Value, lOp1Value, Op1TypeCode);
    ...
    public class INT : Instruct
    {
        ...
        public override void Impl()
        {
            ...
            UInt32 lTempEFLAGS, lNewSS, lNewESP;
            sIDTEntry lIDT;
            sGDTEntry lGDT;
    ...
    public class LODSB : Instruct
    {
        ...
        public override void Impl()
        {
            ...
            do
            {
                ...
                MOVSW.IncDec(mProc, ref lJunk, ref lSource, lAddrSize16, 1, true, false);
    ...
    public class LODSW : Instruct
    {
        ...
        public override void Impl()
        {
            ...
            do
            {
                ...
                MOVSW.IncDec(mProc, ref lJunk, ref lSource, lAddrSize16, 2, true, false);
                //If we are looping and the processor needs attention, break out of the loop without messing up the 
                //repeat condition so the loop can be resumed after the CPU is serviced
                //OR if we weren't meant to loop in the first place
                if ((mProc.mRepeatCondition == Processor_80x86.NOT_REPEAT ||
    ...
    public class LODSD : Instruct
    {
        ...
        public override void Impl()
        {
            ...
            do
            {
                ...
                MOVSW.IncDec(mProc, ref lJunk, ref lSource, lAddrSize16, 4, true, false);
                //If we are looping and the processor needs attention, break out of the loop without messing up the 
                //repeat condition so the loop can be resumed after the CPU is serviced
                //OR if we weren't meant to loop in the first place
                if ((mProc.mRepeatCondition == Processor_80x86.NOT_REPEAT ||
    ...
    public class MOVSB : Instruct
    {
        ...
        public override void Impl()
        {
            ...
            do
            {
                ...
                MOVSW.IncDec(mProc, ref lDest, ref lSource, lAddrSize16, 1, true, true);
    ...
    public class MOVSW : Instruct
    {
        ...
        public override void Impl()
        {
            ...
            SetupMOVSSrcDest(mProc, ref lSource, ref lDest, lAddrSize16);
            ...
            do
            {
                ...
                MOVSW.IncDec(mProc, ref lDest, ref lSource, lAddrSize16, 2,true, true);
                //If we are looping and the processor needs attention, break out of the loop without messing up the 
                //repeat condition so the loop can be resumed after the CPU is serviced
                //OR if we weren't meant to loop in the first place
                if ((mProc.mRepeatCondition == Processor_80x86.NOT_REPEAT) || (mProc.NeedToInterruptLoop() && mProc.mRepeatCondition != Processor_80x86.NOT_REPEAT))
        ...
        public static void IncDec(Processor_80x86 mProc, ref DWord Dest, ref DWord Src, bool AddrSize16, byte Size, bool IncSource, bool IncDest)
        {
            DWord Junk = 0;
            ...
            if (AddrSize16)
                if (mProc.regs.FLAGSB.DF)
                {
                    ...
                    if (IncDest)
                        mProc.regs.DI -= Size;
                    if (mProc.regs.SI <= 1 || mProc.regs.DI <= 1 || mProc.regs.SI >= 0xfffe || mProc.regs.DI >= 0xfffe)
                        if (IncSource)
                        {
                            if (IncDest)
                                SetupMOVSSrcDest(mProc, ref Src, ref Dest, AddrSize16);
                            else
                                SetupMOVSSrcDest(mProc, ref Src, ref Junk, AddrSize16);
                        }
                        else
                        {
                            if (IncDest)
                                SetupMOVSSrcDest(mProc, ref Junk, ref Dest, AddrSize16);
                        }
                    else
                    {
                        if (IncSource)
                ...
                if (mProc.regs.FLAGSB.DF)
                {
                    ...
                    if (IncDest)
                        mProc.regs.EDI -= Size;
                    if (mProc.regs.ESI <= 1 || mProc.regs.EDI <= 1 || mProc.regs.ESI >= 0xfffe || mProc.regs.EDI >= 0xfffe)
                        if (IncSource)
                        {
                            if (IncDest)
                                SetupMOVSSrcDest(mProc, ref Src, ref Dest, AddrSize16);
    ...
    public class MOVSD : Instruct
    {
        ...
        public override void Impl()
        {
            ...
            do
            {
                ...
                MOVSW.IncDec(mProc, ref lDest, ref lSource, lAddrSize16, 4, true,true);
                //If we are looping and the processor needs attention, break out of the loop without messing up the 
                //repeat condition so the loop can be resumed after the CPU is serviced
                //OR if we weren't meant to loop in the first place
                if ((mProc.mRepeatCondition == Processor_80x86.NOT_REPEAT) || (mProc.NeedToInterruptLoop() && mProc.mRepeatCondition != Processor_80x86.NOT_REPEAT))
    ...
    public class STOSB : Instruct
    {
        ...
        public override void Impl()
        {
            ...
            do
            {
               ...
               MOVSW.IncDec(mProc, ref lDest, ref lJunk, lAddrSize16, 1, false, true);
                //If we are looping and the processor needs attention, break out of the loop without messing up the 
                //repeat condition so the loop can be resumed after the CPU is serviced
                //OR if we weren't meant to loop in the first place
                if ((mProc.mRepeatCondition == Processor_80x86.NOT_REPEAT ||
    ...
    public class STOSW : Instruct
    {
        ...
        public override void Impl()
        {
            ...
            do
            {
                ...
                MOVSW.IncDec(mProc, ref lDest, ref lJunk, lAddrSize16, 2, false, true);
                //If we are looping and the processor needs attention, break out of the loop without messing up the 
                //repeat condition so the loop can be resumed after the CPU is serviced
                //OR if we weren't meant to loop in the first place
                if ((mProc.mRepeatCondition == Processor_80x86.NOT_REPEAT ||
    ...
    public class STOSD : Instruct
    {
        ...
        public override void Impl()
        {
            ...
            do
            {
                ...
                MOVSW.IncDec(mProc, ref lDest, ref lJunk, lAddrSize16, 4, false,true);
                //If we are looping and the processor needs attention, break out of the loop without messing up the 
                //repeat condition so the loop can be resumed after the CPU is serviced
                //OR if we weren't meant to loop in the first place
                if ((mProc.mRepeatCondition == Processor_80x86.NOT_REPEAT ||
