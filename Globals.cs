...
namespace VirtualProcessor
{
    ...
    public struct sDebugComponents
    {
        ...
        public sDebugComponents(bool Kbd, bool CPU, bool PIC, bool FDC, bool CMOS, bool DMA, bool HDC, bool MemPaging, bool Serial, bool Pit, bool Exceptions)
        {
            DebugKbd = Kbd;
            DebugCPU = CPU;
            DebugPIC = PIC;
...
//            Console.WriteLine("Wrote CMOS to " + CMOSFile);
        ...
        public static string GetLinuxCurrentTaskName(PCSystem mSystem)
        {
            ...
            if (lstartAddr == 0xf1f1f1f1 || sIns.ExceptionThrown)
                return "Paging Error";
            for (UInt32 cnt = lstartAddr; cnt < lstartAddr + mSystem.TaskNameOffset; cnt++)
            {
                byte lTemp = mSystem.mProc.mem.pMemory(mSystem.mProc, cnt);
                if (lTemp > 19 && lTemp < 127)
                    lJobName += (char)lTemp;
                else
                    break;
            }
