...
namespace VirtualProcessor
{
    ...
    public struct sInstruction
    {
        ...
        [MarshalAs(UnmanagedType.U1)]
        public bool InstructionOpSize16, InstructionAddSize16, Lock, RepNZ, RepZ, IsTwoByteOpCode, HasModRM, HasSIB;
        [MarshalAs(UnmanagedType.U1)]
    ...
    public class Decoder
    {
        ...
        private sInstruction mInstruction;
        ...
        private sOpCode mChosenOpCode;
        private readonly InstructionList mIList;
        ...
        public Decoder(Processor_80x86 Proc, InstructionList IList, sOpCodePointer[] OpcodeIndexer, bool UseProcessorCS_IP, bool TriggerExceptions)
        {
            mProc = Proc;
            mIList = IList;
        ...
        public Instruct Decode()
        {
            ...
            mProcessor_ProtectedModeActive = mProc.ProtectedModeActive;
...
//            int a = 1;
//            if ((mProc.regs.CS.Value == 0x15F400) && mProc.regs.EIP == 0x00001EC4)
//                a = a + 1 - 5 + 6;

            //If bool set we use processor's CS_IP, otherwise we assume that the current mDecodeOffset is correct
            if (mUseProcessorCS_IP)
            {
                if (mProc.OperatingMode == ProcessorMode.Protected && mProc.regs.CS.DescriptorNum > 0)
            ...
            if (mInstruction.ExceptionThrown) { if (!mTriggerExceptions) mInstruction.ExceptionThrown = false; mValidDecode = false; mChosenInstruction.DecodedInstruction = mInstruction; return mChosenInstruction; }
            #region Opcode Processing
            //Get the OpCode
            if (mTempPrefixByteUsed)
                mInstruction.OpCode = mProc.mem.GetByte(mProc, ref mInstruction, (UInt32)mDecodeOffset++);
            else
            {
            ...
            {
                byte lNextByte = mProc.mem.GetByte(mProc, ref mInstruction, mDecodeOffset);
                if (mInstruction.ExceptionThrown) { if (!mTriggerExceptions) mInstruction.ExceptionThrown = false; mValidDecode = false; mChosenInstruction.DecodedInstruction = mInstruction; return mChosenInstruction; }
                ...
                if (mInstruction.OpCode == 0x0F || (lNextByte > 0xBF))
                {
                    mInstruction.OpCode = (UInt16)(mInstruction.OpCode << 8 | lNextByte);
                ...
                {
                    mInstruction.OpCode = (mInstruction.OpCode << 8) | (byte)( (lNextByte >>3) & 0x07);
            ...
            try
            {
                //mChosenInstruction = Instruct.NewInstruct(mProc, mOpcodeIndexer[(int)mInstruction.OpCode].InstructionName);
                mChosenInstruction = mIList[mInstruction.OpCode];
            ...
            mChosenOpCode = mProc.OpCodeIndexer[mInstruction.OpCode].OpCode;
            mChosenInstruction.ChosenOpCode = mChosenOpCode;
            mOpFound = true;
            if (mChosenOpCode.OpCode == 0xFFFF)
                mOpFound = false;
            ...
            if (mChosenOpCode.Op1AM != sOpCodeAddressingMethod.None)
                switch (mChosenOpCode.Op1AM)
                {
                    ...
                    case sOpCodeAddressingMethod.NamedRegister:     //Register named in Operand definition
                        mInstruction.Op1.Register = GetRegEnumForOpcodeNamedByOpSize(mChosenOpCode.Register1); break;
                    ...
                    case sOpCodeAddressingMethod.MemoryOnly:        //M - The ModR/M byte may refer only to memory (for example, BOUND, LES, LDS, LSS, LFS, LGS, CMPXCHG8B).
                        ...
                        if (mInstruction.OverrideSegment == 0 && mOverrideSegment != eGeneralRegister.NONE && mChosenOpCode.Op1OT != sOpCodeOperandType.None)
                        {
                    ...
                    case sOpCodeAddressingMethod.GenReg:            //G - The reg field of the ModR/M byte selects a general register 
                        mInstruction.Op1.Register = GetRegEnumForOpType(mTemp_MODRM_REGFieldRegister, mChosenOpCode.Op1OT); break;
                    ...
                    case sOpCodeAddressingMethod.ModGenReg:         //R - The mod field of the ModR/M byte may refer only to a general register
                        mInstruction.Op1.Register = GetRegEnumForOpType(mTemp_MODRM_MODFieldRegister, mChosenOpCode.Op1OT); break;
                    case sOpCodeAddressingMethod.RegSegReg:         //S - The reg field of the ModR/M byte selects a segment register
                        mInstruction.Op1.Register = GetModRMSegReg(); break;
                    ...
                    default:
                        throw new Exception("Uncoded Addressing Method: " + mChosenOpCode.Op1AM);
                }
            #endregion
            #region Operand 2 Processing
            if (mChosenOpCode.Op2AM != sOpCodeAddressingMethod.None)
                switch (mChosenOpCode.Op2AM)
                {
                    case sOpCodeAddressingMethod.TheNumberOne:
                    ...
                    case sOpCodeAddressingMethod.JmpRelOffset:      //J - J	The instruction contains a relative offset to be added to the instruction pointer register
                        ...
                        break;
                    case sOpCodeAddressingMethod.NamedRegister:     //Register named in Operand definition
                        mInstruction.Op2.Register = GetRegEnumForOpcodeNamedByOpSize(mChosenOpCode.Register2); break;
                    case sOpCodeAddressingMethod.DirectAddress:     //A - Direct address. The instruction has no ModR/M byte; the address of the operand is encoded in the instruction; and no base register, index register, or scaling factor can be applied (for example, far JMP (EA)).
                        #region Direct Address
                        switch (mChosenOpCode.Op2OT)
                        {
                    ...
                    case sOpCodeAddressingMethod.MemoryOnly:        //M - The ModR/M byte may refer only to memory (for example, BOUND, LES, LDS, LSS, LFS, LGS, CMPXCHG8B).
                        ...
                        if (mInstruction.OverrideSegment == 0 && mOverrideSegment != eGeneralRegister.NONE && mChosenOpCode.Op2OT != sOpCodeOperandType.None)
                        {
                            mInstruction.OverrideSegment = mOverrideSegment;
                        }
                    ...
                    case sOpCodeAddressingMethod.EType:             //E - A ModR/M byte follows the opcode and specifies the operand. The operand is either a general-purpose register or a memory address. If it is a memory address, the address is computed from a segment register and any of the following values: a base register, an index register, a scaling factor, a displacement.
                        ...
                        mInstruction.Op2.Register = GetRegEnumForOpType(mTemp_MODRM_MODFieldRegister, mChosenOpCode.Op2OT);
                        if (mInstruction.OverrideSegment == 0 && mOverrideSegment != eGeneralRegister.NONE && mChosenOpCode.Op1OT != sOpCodeOperandType.None)
                        {
                            mInstruction.OverrideSegment = mOverrideSegment;
                        }
                        ...
                        break;
                    case sOpCodeAddressingMethod.GenReg:            //G - The reg field of the ModR/M byte selects a general register 
                        mInstruction.Op2.Register = GetRegEnumForOpType(mTemp_MODRM_REGFieldRegister, mChosenOpCode.Op2OT); break;
                    case sOpCodeAddressingMethod.MMXPkdQWord:       //P - The reg field of the ModR/M byte selects a packed quadword MMX technology register.
                        GetRegEnumForMMXReg(); break;
                    case sOpCodeAddressingMethod.QType:             //Q - A ModR/M byte follows the opcode and specifies the operand. The operand is either an MMX technology register or a memory address. If it is a memory address, the address is computed from a segment register and any of the following values: a base register, an index register, a scaling factor, and a displacement.
                        #region SetC up register via ModMR/SIB using Reg as MMX Register
                    ...
                    //}
                        #endregion
                    case sOpCodeAddressingMethod.ModGenReg:         //R - The mod field of the ModR/M byte may refer only to a general register
                        mInstruction.Op2.Register = GetRegEnumForOpType(mTemp_MODRM_MODFieldRegister, mChosenOpCode.Op2OT); break;
                    case sOpCodeAddressingMethod.RegSegReg:         //S - The reg field of the ModR/M byte selects a segment register
                        mInstruction.Op2.Register = GetModRMSegReg(); break;
                    case sOpCodeAddressingMethod.RegTestReg:        //T
                    ...
                    case sOpCodeAddressingMethod.None: break;
                    default:
                        throw new Exception("Uncoded Addressing Method: " + mChosenOpCode.Op2AM);
                }
            #endregion
            #region Operand 3 Processing
            switch (mChosenOpCode.Op3AM)
            {
                ////Operand 3 can only be an immediate?
                case sOpCodeAddressingMethod.TheNumberOne:
                    mInstruction.Op3.Imm8 = 1; mInstruction.Op3.HasImm8 = true; break;
                case sOpCodeAddressingMethod.NamedRegister:     //Register named in Operand definition
                    mInstruction.Op3.Register = GetRegEnumForOpcodeNamedByOpSize(mChosenOpCode.Register3); break;
                case sOpCodeAddressingMethod.ImmedData:         //I
                    #region Get byte(s) immediately following the instruction
                    switch (mChosenOpCode.Op3OT)
                    {
...
#endif
            mChosenInstruction.DecodedInstruction = mInstruction;
            //System.Diagnostics.Debug.WriteLine("");
            mChosenInstruction.DecodedInstruction.InstructionOpSize16 = !mInstruction.OpSizePrefixFound;
            mChosenInstruction.DecodedInstruction.InstructionAddSize16 = !mInstruction.AddrSizePrefixFound;
            if (mChosenInstruction == null)
                System.Diagnostics.Debugger.Break();
            mInstruction.mInstructionStatus = eProcessorStatus.NotDecoding;
            return mChosenInstruction;
        }

        private bool DecodePrefix(Decoder d)
        {
            //byte mDecodeOffsetPlus1 = 0;
            //TODO: Need to add D8-DF - ALU instructions
            d.mTempPrefixByte = d.mProc.mem.GetByte(d.mProc, ref d.mInstruction, d.mDecodeOffset);
            d.mTempPrefixByteUsed = false;
            switch (d.mTempPrefixByte)
            {
                case 0x26:  //26 = ES seg override
                    d.mInstruction.OverrideSegment = eGeneralRegister.ES;
                    return true;
                case 0x64:  //64 = FS seg override
                    d.mInstruction.OverrideSegment = eGeneralRegister.FS;
                    return true;
                case 0x65:  //65 = GS seg override
                    d.mInstruction.OverrideSegment = eGeneralRegister.GS;
                    return true;
                case 0x36:  //36 = SS seg override
                    d.mInstruction.OverrideSegment = eGeneralRegister.SS;
                    return true;
                case 0x2E:  //2E = CS seg override or Hint
                    ...
                    //    return true;
                    //}
                    d.mInstruction.OverrideSegment = eGeneralRegister.CS;
                    return true;
                case 0x3E:  //3E = DS seg override or Hint
                    ...
                    //    return true;
                    //}
                    d.mInstruction.OverrideSegment = eGeneralRegister.DS;
                    return true;
                case 0x66:  //66 = Operand size override prefix
                    d.mProcessor_OpSize16 = !d.mOrigProcessor_OpSize16;
                    d.mInstruction.OpSizePrefixFound = true;
                    return true;
                case 0x67:  //67 = Address-size override prefix
                    d.mProcessor_AddrSize16 = !d.mOrigProcessor_AddrSize16;
                    d.mInstruction.AddrSizePrefixFound = true;
                    return true;
                case 0xF0:
                    d.mInstruction.Lock = true;
                    return true;
                case 0xF2:
                    d.mInstruction.RepNZ = true;
                    return true;
                case 0xF3:
                    d.mInstruction.RepZ = true;
                    return true;
            }
            return false;
        }

        private void GetModRMAndSIB()
        {
            UInt32 lRealOpCode = mInstruction.RealOpCode;

            UInt32 lTemp = mInstruction.OpCode & 0xFF00;
            if ((lTemp == 0xD800 || lTemp == 0xD900
                   || lTemp == 0xDA00 || lTemp == 0xDB00
                   || lTemp == 0xDC00 || lTemp == 0xDD00
                   || lTemp == 0xDE00 || lTemp == 0xDF00))
        ...
        private void ProcessModRMAndSIB()
        {
            if (mProcessor_AddrSize16)
                PreProcessModRM16();
            else
                ...
                //    PreProcessSIB();
                //else
                    //ModRM32 code calls PreProcessSIB when necessary
                    PreProcessModRM32();
        }

        private void PreProcessModRM16()
        {
                ...
                switch (mInstruction.ModRegRM_Mod)
                {
                    ...
                    case 3:
                        switch (mInstruction.ModRegRM_RM)
                        {
                            ...
                            case 0: mTemp_MODRM_MODFieldRegister = GetRegEnumForSetWBit(eGeneralRegister.AL); break;
                            case 1: mTemp_MODRM_MODFieldRegister = GetRegEnumForSetWBit(eGeneralRegister.CL); break;
                            case 2: mTemp_MODRM_MODFieldRegister = GetRegEnumForSetWBit(eGeneralRegister.DL); break;
                            case 3: mTemp_MODRM_MODFieldRegister = GetRegEnumForSetWBit(eGeneralRegister.BL); break;
                            case 4: mTemp_MODRM_MODFieldRegister = GetRegEnumForSetWBit(eGeneralRegister.AH); break;
                            case 5: mTemp_MODRM_MODFieldRegister = GetRegEnumForSetWBit(eGeneralRegister.CH); break;
                            case 6: mTemp_MODRM_MODFieldRegister = GetRegEnumForSetWBit(eGeneralRegister.DH); break;
                            case 7: mTemp_MODRM_MODFieldRegister = GetRegEnumForSetWBit(eGeneralRegister.BH); break;
                        }
                        break;
                }
            #endregion
            ...
            {//mTempREGFieldReg
                case 0: mTemp_MODRM_REGFieldRegister = GetRegEnumForSetWBit(eGeneralRegister.AL); break;
                case 1: mTemp_MODRM_REGFieldRegister = GetRegEnumForSetWBit(eGeneralRegister.CL); break;
                case 2: mTemp_MODRM_REGFieldRegister = GetRegEnumForSetWBit(eGeneralRegister.DL); break;
                case 3: mTemp_MODRM_REGFieldRegister = GetRegEnumForSetWBit(eGeneralRegister.BL); break;
                case 4: mTemp_MODRM_REGFieldRegister = GetRegEnumForSetWBit(eGeneralRegister.AH); break;
                case 5: mTemp_MODRM_REGFieldRegister = GetRegEnumForSetWBit(eGeneralRegister.CH); break;
                case 6: mTemp_MODRM_REGFieldRegister = GetRegEnumForSetWBit(eGeneralRegister.DH); break;
                case 7: mTemp_MODRM_REGFieldRegister = GetRegEnumForSetWBit(eGeneralRegister.BH); break;
            }
            #endregion

        }

        private void PreProcessModRM32()
        {
        ...
        private void PreProcessSIB()
        {
            ...
            switch (mInstruction.ModRegRM_Mod)
            {
                ...
                case 1:
                case 2:
                        mTempEffReg1 = IdentifySIBRegister(mInstruction.Index, 0);
                        mTempEffReg2 = IdentifySIBRegister(mInstruction.Base, 1);
                    break;
                case 3:
                    switch (mInstruction.ModRegRM_RM)
                    {//These will be fixed up later
        ...
        private eGeneralRegister IdentifySIBRegister(byte ToId, int Type)
        {
            switch (ToId)
            {
        ...
        private eGeneralRegister IdentifyDebugRegister(byte eeeField)
        {
            switch (eeeField)
            {
                case 0:
        ...
        private eGeneralRegister IdentifyControlRegister(byte eeeField)
        {
            switch (eeeField)
            {
                case 0:
        ...
        public eGeneralRegister GetModRMSegReg()
        {
            switch (mInstruction.ModRegRM_Reg)
            {
        ...
        public eGeneralRegister GetModRMControlReg()
        {
            switch (mInstruction.ModRegRM_Reg)
            {
        ...
        public eGeneralRegister GetModRMDebugReg()
        {
            switch (mInstruction.ModRegRM_Reg)
            {
        ...
        public eGeneralRegister GetRegEnumForSetWBit(eGeneralRegister Reg)
        {
            if (!mInstruction.OpSizeBit)
                return Reg;
            switch (Reg)
            {
        ...
        public eGeneralRegister GetRegEnumForOpcodeNamedByOpSize(eGeneralRegister Reg)
        {
            ...
            switch (Reg)
            {
                case eGeneralRegister.EAX: return (mProcessor_OpSize16 ? eGeneralRegister.AX : Reg);
                case eGeneralRegister.ECX: return (mProcessor_OpSize16 ? eGeneralRegister.CX : Reg);
                case eGeneralRegister.EDX: return (mProcessor_OpSize16 ? eGeneralRegister.DX : Reg);
                case eGeneralRegister.EBX: return (mProcessor_OpSize16 ? eGeneralRegister.BX : Reg);
                case eGeneralRegister.ESP: return (mProcessor_OpSize16 ? eGeneralRegister.SP : Reg);
                case eGeneralRegister.EBP: return (mProcessor_OpSize16 ? eGeneralRegister.BP : Reg);
                case eGeneralRegister.ESI: return (mProcessor_OpSize16 ? eGeneralRegister.SI : Reg);
                case eGeneralRegister.EDI: return (mProcessor_OpSize16 ? eGeneralRegister.DI : Reg);
                case eGeneralRegister.AX: return (!mProcessor_OpSize16 ? eGeneralRegister.EAX : Reg);
                case eGeneralRegister.CX: return (!mProcessor_OpSize16 ? eGeneralRegister.ECX : Reg);
                case eGeneralRegister.DX: return (!mProcessor_OpSize16 ? eGeneralRegister.EDX : Reg);
                case eGeneralRegister.BX: return (!mProcessor_OpSize16 ? eGeneralRegister.EBX : Reg);
                case eGeneralRegister.SP: return (!mProcessor_OpSize16 ? eGeneralRegister.ESP : Reg);
                case eGeneralRegister.BP: return (!mProcessor_OpSize16 ? eGeneralRegister.EBP : Reg);
                case eGeneralRegister.SI: return (!mProcessor_OpSize16 ? eGeneralRegister.ESI : Reg);
                case eGeneralRegister.DI: return (!mProcessor_OpSize16 ? eGeneralRegister.EDI : Reg);
                case eGeneralRegister.AL: return eGeneralRegister.AL;
                case eGeneralRegister.DL: return eGeneralRegister.DL;
                case eGeneralRegister.CL: return eGeneralRegister.CL;
                case eGeneralRegister.BL: return eGeneralRegister.BL;
                case eGeneralRegister.AH: return eGeneralRegister.AH;
                case eGeneralRegister.DH: return eGeneralRegister.DH;
                case eGeneralRegister.CH: return eGeneralRegister.CH;
                case eGeneralRegister.BH: return eGeneralRegister.BH;
                case eGeneralRegister.CS: return eGeneralRegister.CS;
                case eGeneralRegister.DS: return eGeneralRegister.DS;
                case eGeneralRegister.ES: return eGeneralRegister.ES;
                case eGeneralRegister.FS: return eGeneralRegister.FS;
                case eGeneralRegister.GS: return eGeneralRegister.GS;
                case eGeneralRegister.SS: return eGeneralRegister.SS;
            }
