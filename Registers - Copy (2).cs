...
namespace VirtualProcessor
{
    ...
    public struct sSegmentSelector
    {
        ...
        private sGDTEntry mSelector, mPrevSelector;
        ...
        public UInt32 Value
        {
            get
            {
                if (mProc.OperatingMode == ProcessorMode.Protected && mDescriptorNum > 0)
                {
                    if (mSelector.access.SystemDescType == eSystemOrGateDescType.TSS_32_Av || mSelector.access.SystemDescType == eSystemOrGateDescType.TSS_32_Bu)
                    {
            ...
            set
            {
                ...
                {   //Expecting to receive a Descriptor number, NOT a segment address
                    ...
                    if (mWhichReg == eGeneralRegister.CS)
                        mProc.regs.CPL = mSelector.access.PrivLvl;
                }
                else
   ...
   public struct RegStruct
    {
        public RegStruct(Processor_80x86 proc)
        {
            ...
            EIP = EAX = EBX = ECX = EDX = ESI = EDI = EBP = ESP = EFLAGS = mCR0 = CR2 = mCR3 = mCR4 = DR0 = DR1 = DR2 = DR3 = DR6 = DR7 = 0;
            CPL = ePrivLvl.Kernel_Ring_0;
        }
       ...
       [FieldOffset(2000)]
        ...
        {
            byte lInVal = (byte)(InVal & 0x000000FF);
            if (lInVal == 0)
            {
                    //FLAGS &= 0xFFFB;
                //According to http://stackoverflow.com/questions/15959798/parity-flag-for-value-of-0-in-x86:
                //0 is an even number, so, the answer is yes.
                FLAGS |= 0x4;
                return;
            }
            //08/31/10 - updated for performance
            bool lParity=true;
            //NOTE: In x86 processors, the parity flag reflects the parity only of the least significant byte of the result
            for (int cnt = 0; cnt <= 7; cnt++)
                //if ((lInVal & (byte)Math.Pow(2, cnt)) == Math.Pow(2, cnt))
                if (( (lInVal & (1 << cnt)) == 1 << cnt))
                    lParity = !lParity;
            if (lParity)
                FLAGS |= 0x4;
            else
                FLAGS &= 0xFFFB;
            //NOTE: This should have been / all along, not %
            //TODO: Figure out why this only works when it is wrong
            //Should be % not /
            //setFlagPF( (lTemp % 2) == 0);
        }
        ...
        public void setFlagOF_Add(byte PreVal1, byte PreVal2, byte PostVal)
        {
            if ((byte)(((PreVal1 ^ PreVal2 ^ 0x80) & (PostVal ^ PreVal1)) & 0x80) > 0)
                ...
                FLAGS &= 0xF7FF;
return;
            if (PreVal1 == 0 && PreVal2 == 0 && PostVal == 0)
            {
                FLAGS &= 0xF7FF;
                return;
            }

            byte lPreBit1 = (byte)((PreVal1 & 0x80) >> 7);
            byte lPreBit2 = (byte)((PreVal2 & 0x80) >> 7);
            byte lPostBit = (byte)((PostVal & 0x80) >> 7); ;

            if ((byte)(PreVal1 + PreVal2) == PostVal)
                if ((lPreBit1 == 0 && lPreBit2 == 0 && lPostBit == 1)
                     )
                    FLAGS |= 0x800;
                else
                    FLAGS &= 0xF7FF;
            else 
                if ((lPreBit1 == 0 && lPreBit2 == 1 && lPostBit == 1)
                     )
                    FLAGS |= 0x800;
                else
                    FLAGS &= 0xF7FF;
        ...
        public void setFlagOF_Add(UInt16 PreVal1, UInt16 PreVal2, UInt16 PostVal)
        {
            ...
            byte lPreBit1 = (byte)((PreVal1 & 0x8000) >> 15);
            byte lPreBit2 = (byte)((PreVal2 & 0x8000) >> 15);
            byte lPostBit = (byte)((PostVal & 0x8000) >> 15); ;

            if ((UInt16)(PreVal1 + PreVal2) == PostVal)
                if ((lPreBit1 == 0 && lPreBit2 == 0 && lPostBit == 1)
                    || (lPreBit1 == 1 && lPreBit2 == 1 && lPostBit == 0))
                    FLAGS |= 0x800;
                else
                    FLAGS &= 0xF7FF;
            else if ((UInt16)(PreVal1 - PreVal2) == PostVal)
                if ((lPreBit1 == 0 && lPreBit2 == 1 && lPostBit == 1)
                    || (lPreBit1 == 1 && lPreBit2 == 0 && lPostBit == 0))
                    FLAGS |= 0x800;
                else
                    FLAGS &= 0xF7FF;
        }
        public void setFlagOF_Add(UInt32 PreVal1, UInt32 PreVal2, UInt32 PostVal)
        {
            ...
            byte lPreBit1 = (byte)((PreVal1 & 0x80000000) >> 31);
            byte lPreBit2 = (byte)((PreVal2 & 0x80000000) >> 31);
            byte lPostBit = (byte)((PostVal & 0x80000000) >> 31); ;

            if ((UInt32)(PreVal1 + PreVal2) == PostVal)
                if ((lPreBit1 == 0 && lPreBit2 == 0 && lPostBit == 1)
                    || (lPreBit1 == 1 && lPreBit2 == 1 && lPostBit == 0))
                    FLAGS |= 0x800;
                else
                    FLAGS &= 0xF7FF;
            else if ((UInt32)(PreVal1 - PreVal2) == PostVal)
                if ((lPreBit1 == 0 && lPreBit2 == 1 && lPostBit == 1)
                    || (lPreBit1 == 1 && lPreBit2 == 0 && lPostBit == 0))
                    FLAGS |= 0x800;
                else
        ...
        public void setFlagOF_Add(UInt64 PreVal1, UInt64 PreVal2, UInt64 PostVal)
        {
            ...
            byte lPreBit1 = (byte)((PreVal1 & 0x8000000000000000) >> 63);
            byte lPreBit2 = (byte)((PreVal2 & 0x8000000000000000) >> 63);
            byte lPostBit = (byte)((PostVal & 0x8000000000000000) >> 63); ;

            if ((UInt64)(PreVal1 + PreVal2) == PostVal)
                if ((lPreBit1 == 0 && lPreBit2 == 0 && lPostBit == 1)
                    || (lPreBit1 == 1 && lPreBit2 == 1 && lPostBit == 0))
                    FLAGS |= 0x800;
                else
                    FLAGS &= 0xF7FF;
            else if ((UInt64)(PreVal1 - PreVal2) == PostVal)
                if ((lPreBit1 == 0 && lPreBit2 == 1 && lPostBit == 1)
                    || (lPreBit1 == 1 && lPreBit2 == 0 && lPostBit == 0))
                    FLAGS |= 0x800;
                else
